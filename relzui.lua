if (game:GetService("CoreGui")):FindFirstChild("NanoXHub") and (game:GetService("CoreGui")):FindFirstChild("ScreenGui") then
	(game:GetService("CoreGui")).NanoXHub:Destroy();
	(game:GetService("CoreGui")).ScreenGui:Destroy();
end;
_G.Primary = Color3.fromRGB(100, 100, 100);
_G.Dark = Color3.fromRGB(22, 22, 26);
_G.Third = Color3.fromRGB(189, 207, 70);
...
ImageButton.MouseButton1Click:connect(function()
	(game.CoreGui:FindFirstChild("NanoXHub")).Enabled = not (game.CoreGui:FindFirstChild("NanoXHub")).Enabled;
end);
...
(getgenv()).LoadConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("Nano X Hub") then
			makefolder("Nano X Hub");
		end;
		if not isfolder("Nano X Hub/Library/") then
			makefolder("Nano X Hub/Library/");
		end;
		if not isfile(("Nano X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("Nano X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(SettingsLib));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("Nano X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json"));
...
(getgenv()).SaveConfig = function()
	if readfile and writefile and isfile and isfolder then
		if not isfile(("Nano X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json")) then
			...
			writefile("Nano X Hub/Library/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
		end;
	else
		return warn("Status : Undetected Executor");
	end;
...
function Update:Window(Config)
	...
	local NanoXHub = Instance.new("ScreenGui");
	NanoXHub.Name = "NanoXHub";
	NanoXHub.Parent = game.CoreGui;
	NanoXHub.DisplayOrder = 999;
	local OutlineMain = Instance.new("Frame");
	OutlineMain.Name = "OutlineMain";
	OutlineMain.Parent = NanoXHub;
	...
	CreateButton("Reset Config", function()
		if isfolder("Nano X Hub") then
			delfolder("Nano X Hub");
		end;
